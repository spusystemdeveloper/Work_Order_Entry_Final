/*
  Work Order Entry / Order Processing Queueing database object
  Read-only source export: DVO_STORE_DB
  Object: [dbo].[SOD_fn_GetConvertQty]
  Source type: FN
  Review branch-specific database, StoreID, warehouse, and business-rule references before deployment.
*/
SET ANSI_NULLS ON;
GO
SET QUOTED_IDENTIFIER ON;
GO
SET XACT_ABORT ON;
GO

DECLARE @ExistingType char(2) =
    (SELECT type FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SOD_fn_GetConvertQty]'));

IF @ExistingType IS NOT NULL AND @ExistingType <> 'FN'
    THROW 52001, 'Object type conflict for [dbo].[SOD_fn_GetConvertQty].', 1;

IF @ExistingType IS NULL
    EXEC sys.sp_executesql N'
CREATE FUNCTION [dbo].[SOD_fn_GetConvertQty]
(
	@vLookUp varchar(30),
	@vTotal BIT
)
RETURNS float
AS
BEGIN
	
	DECLARE @ID int,
		@ItemLookUpCode varchar(30),
		@ParentItem int,
		@ParentQuantity float,
		@Quantity float,
		@Committed float,
		@SKULevel int,
		@Available float,
		@xLoop BIT
	
	SET @Available = 0
	SET @xLoop = 0

	DECLARE cur CURSOR FOR

	SELECT ID,ItemLookUpcode,ParentItem,ParentQuantity,Quantity,QuantityCommitted,CONVERT(INT,SubDescription2) AS SKULEVEL
	FROM Item WHERE ItemLookUpCode LIKE ''%'' + Left(@vLookUp,5) + ''%'' 
	ORDER BY CONVERT(INT,SubDescription2) DESC
	
	OPEN CUR
	FETCH next FROM cur INTO @ID,@ItemLookUpCode,@ParentItem,@ParentQuantity,@Quantity,@Committed,@SKULevel
	WHILE @xLoop != 1
	BEGIN 
		
		IF @vTotal = 1 BEGIN

			SET @Available = (@Available * @ParentQuantity) + (@Quantity - @Committed)
			IF @ItemLookUpCode = @vLookUp
			BEGIN
				SET @xLoop = 1
			END

		END ELSE BEGIN

			SET @Available = (@Available * @ParentQuantity) + (@Quantity - @Committed)

			IF @SKULevel = 1 BEGIN
				SET @xLoop = 1
			END

		END

	FETCH next FROM cur INTO @ID,@ItemLookUpCode,@ParentItem,@ParentQuantity,@Quantity,@Committed,@SKULevel
	END
	CLOSE cur
	DEALLOCATE cur

	RETURN @Available

END
';
ELSE
    EXEC sys.sp_executesql N'
ALTER FUNCTION [dbo].[SOD_fn_GetConvertQty]
(
	@vLookUp varchar(30),
	@vTotal BIT
)
RETURNS float
AS
BEGIN
	
	DECLARE @ID int,
		@ItemLookUpCode varchar(30),
		@ParentItem int,
		@ParentQuantity float,
		@Quantity float,
		@Committed float,
		@SKULevel int,
		@Available float,
		@xLoop BIT
	
	SET @Available = 0
	SET @xLoop = 0

	DECLARE cur CURSOR FOR

	SELECT ID,ItemLookUpcode,ParentItem,ParentQuantity,Quantity,QuantityCommitted,CONVERT(INT,SubDescription2) AS SKULEVEL
	FROM Item WHERE ItemLookUpCode LIKE ''%'' + Left(@vLookUp,5) + ''%'' 
	ORDER BY CONVERT(INT,SubDescription2) DESC
	
	OPEN CUR
	FETCH next FROM cur INTO @ID,@ItemLookUpCode,@ParentItem,@ParentQuantity,@Quantity,@Committed,@SKULevel
	WHILE @xLoop != 1
	BEGIN 
		
		IF @vTotal = 1 BEGIN

			SET @Available = (@Available * @ParentQuantity) + (@Quantity - @Committed)
			IF @ItemLookUpCode = @vLookUp
			BEGIN
				SET @xLoop = 1
			END

		END ELSE BEGIN

			SET @Available = (@Available * @ParentQuantity) + (@Quantity - @Committed)

			IF @SKULevel = 1 BEGIN
				SET @xLoop = 1
			END

		END

	FETCH next FROM cur INTO @ID,@ItemLookUpCode,@ParentItem,@ParentQuantity,@Quantity,@Committed,@SKULevel
	END
	CLOSE cur
	DEALLOCATE cur

	RETURN @Available

END
';
GO
